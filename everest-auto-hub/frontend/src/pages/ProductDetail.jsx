...
export default function ProductDetail() {
  ...
  useEffect(() => {
    ...
    api.get(`/products/${id}`).then(r => {
      const data = r.data;
      // Ensure JSON fields are arrays (MySQL may return them as strings)
      const safeArr = (v) => Array.isArray(v) ? v : (typeof v === 'string' ? JSON.parse(v || '[]') : []);
      data.images = safeArr(data.images);
      data.sizes  = safeArr(data.sizes);
      data.colors = safeArr(data.colors);
      setProduct(data);
  ...
  return (
    <div className="product-detail-page">
      <SEO
        ...
        description={`${product.description?.slice(0, 150)}. Buy ${product.name} from Everest Auto Hub.`}
        ...
        schema={{
          ...
          image: getImageUrl(product.images?.[0]),
          brand: { '@type': 'Brand', name: product.brand || 'Everest Auto Hub' },
          offers: {
            '@type': 'Offer',
            price: product.price,
            priceCurrency: 'AUD',
