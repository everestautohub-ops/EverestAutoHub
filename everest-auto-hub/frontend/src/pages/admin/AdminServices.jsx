...
const empty = { name: '', description: '', price: '', duration: '', isActive: true, image: '', slotsPerHour: 1 };
...
export default function AdminServices() {
  ...
  const openEdit = (s) => {
    setForm({ name: s.name, description: s.description, price: s.price, duration: s.duration || '', isActive: s.isActive, image: s.image || '', slotsPerHour: s.slotsPerHour || 1 });
    setPreview(s.image || '');
    setEditing(s._id);
    setModal(true);
  };
    ...
    <div>
      ...
      <div className="admin-table-wrap">
        <table className="admin-table">
          <thead>
            <tr><th>Image</th><th>Name</th><th>Price</th><th>Duration</th><th>Slots/Hour</th><th>Status</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {services.map(s => (
              <tr key={s._id}>
                ...
                <td style={{ textAlign: 'center' }}>
                  <span style={{ background: 'rgba(249,115,22,0.12)', border: '1px solid rgba(249,115,22,0.3)', color: 'var(--primary)', padding: '3px 10px', borderRadius: 20, fontSize: '0.8rem', fontWeight: 700 }}>
                    {s.slotsPerHour || 1}
                  </span>
                </td>
              ...
              <div className="grid-2">
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={e => setForm({ ...form, slotsPerHour: parseInt(e.target.value) || 1 })}
                    placeholder="1"
                  />
                  <small style={{ color: 'var(--text-muted)', fontSize: '0.75rem', marginTop: 4, display: 'block' }}>
                    How many cars can be booked per time slot (e.g. 2 = two customers at 10:00 AM)
                  </small>
                </div>
                <div className="form-group">
                  <label>Status</label>
